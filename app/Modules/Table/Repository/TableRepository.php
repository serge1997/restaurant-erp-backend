    ...
    public function findAllWithOrders()
    {
        return $this->getQuery()->select(
            ...
            DB::raw("case 
                    ...
                    when TIMESTAMPDIFF(minute, o.created_at, current_timestamp()) >= 60 and TIMESTAMPDIFF(minute, o.created_at, current_timestamp()) < 1440 then concat(TIMESTAMPDIFF(hour, o.created_at, current_timestamp()), 'H')
                    else concat(FLOOR(TIMESTAMPDIFF(minute, o.created_at, current_timestamp()) / 1440), 'dia(s)')
                end as since
            ")
        )
            ->join("orders as o", "o.table_id", "=", "tables.id")
